...
# Modern GCC (11+) is much stricter than the GCC 7 that NVIDIA used for L4T r32.
# Demote new -Werror classes back to warnings so the 4.9 tree still builds.
export KCFLAGS="$KCFLAGS -Wno-error=maybe-uninitialized -Wno-error=incompatible-pointer-types -Wno-error=array-bounds -Wno-error=stringop-overflow -Wno-error=stringop-truncation -Wno-error=dangling-pointer -Wno-error=use-after-free -Wno-error=address-of-packed-member -Wno-error=zero-length-bounds -Wno-error=restrict -Wno-error=nonnull -Wno-error=address -Wno-error=stringop-overread"
export KCPPFLAGS="$KCPPFLAGS -Wno-error=maybe-uninitialized -Wno-error=incompatible-pointer-types"
export KBUILD_CFLAGS_KERNEL="$KBUILD_CFLAGS_KERNEL -Wno-error"
...
Prepare() {
	...
	if [[ -z `ls -A ${KERNEL_DIR}/kernel-4.9` ]]; then
		git clone -b "${KERNEL_BRANCH}" --single-branch https://github.com/NaGaa95/switch-l4t-kernel-4.9.git "${KERNEL_DIR}/kernel-4.9"
	fi

	if [[ -z $(ls -A ${KERNEL_DIR}/nvidia) ]]; then
		git clone -b ${NV_VER} --single-branch https://github.com/NaGaa95/switch-l4t-kernel-nvidia.git "${KERNEL_DIR}/nvidia"
		git clone -b ${NX_VER} --single-branch https://github.com/NaGaa95/switch-l4t-platform-t210-nx.git "${KERNEL_DIR}/hardware/nvidia/platform/t210/nx"
